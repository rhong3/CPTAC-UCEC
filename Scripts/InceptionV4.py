...
def inception_A(input, train=True):
...
def inception_B(input, train=True):
...
def inception_C(input, train=True):
...
def reduction_A(input, k=192, l=224, m=256, n=384, train=True):
...
def reduction_B(input, train=True):
...
def inceptionv4(input, dropout_keep_prob=0.8, num_classes=1000, is_training=True, scope='InceptionV4'):
    ...
    with tf.variable_scope(scope, 'InceptionV4', [input]):
        ...
        # 5 x Inception A
        for i in range(5):
            x = inception_A(x, train=is_training)
            # Output: 35 * 35 * 256

        # Reduction A
        x = reduction_A(x, k=256, l=256, m=384, n=384, train=is_training)  # Output: 17 * 17 * 896

        # 10 x Inception B
        for i in range(10):
            x = inception_B(x, train=is_training)
            # Output: 17 * 17 * 896
        ...
        # Reduction B
        x = reduction_B(x, train=is_training)  # Output: 8 * 8 * 1792

        # 5 x Inception C
        for i in range(5):
            x = inception_C(x, train=is_training)
            # Output: 8 * 8 * 1792
