...
MUT = lbtb[,2:25]
MUT = subset(MUT, select = -c(MLH1, ERBB2, EGFR, PIK3R2, FAT4, ARID5B))
dat = MUT
dat$label = MSIlb
...
cv.out =cv.glmnet(as.matrix(train[,1:18]), as.matrix(train$label),alpha =1,nfolds=10)
plot(cv.out)
cv.out$lambda.min
```
...
```{r}
# Fitting
model =glmnet(as.matrix(train[,1:18]),as.matrix(train$label),alpha=1,lambda=c(cv.out$lambda.min))
# Predict results
results_prob <- predict(model, as.matrix(subset(test,select=c(1:18))),type='response')
# Actual answers
...
```{r}
# Predict results
results_prob <- predict(binary.model, test[,1:18], type='class')
# Actual answers
answers <- factor(test$label, levels=c(0,1))
...
# Predict results
results_prob <- predict(rf.model, newdata = test[,1:18], type='class')
# Actual answers
answers <- factor(test$label, levels=c(0,1))
results <- round(results_prob)
results <- factor(results, levels=c(0,1))
...
# Predict results
results_prob <- predict(gbm.model, newdata = test[,1:18], type="response",n.trees =300)
...
# Predict results
results_prob <- predict(svm.model, newdata = test[,1:18], type="C-classification")
# Actual answers
answers <- factor(test$label, levels=c(0,1))
