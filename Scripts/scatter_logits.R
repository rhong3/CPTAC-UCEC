...
for (i in c('NL5', 'NL6')){
  for (j in c("I1", "I2", "I3", "I5", "I6", "X1", "X2", "X3", "X4", "F1", "F2", "F3", "F4")){
    ...
    ggplot(Slogits_mean, aes(mean_logit0, mean_logit1, color=histology)) + 
      geom_point() + xlab("logit0 mean")+ ylab("logit1 mean") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                                                                   panel.grid.minor = element_blank(), 
                                                                                   axis.line = element_line(colour = "black"), 
                                                                                   legend.position='bottom', legend.box="vertical") + guides(colour = guide_legend(nrow = 4))
    ggsave(paste("Results/NYU_test_revision/", j, "his_NYU_", i, "/out/slide_logits_mean.png", sep=''), plot=last_plot(), width = 5, height = 5, units="in", dpi=300)
    ...
    ggplot(Tlogits_mean, aes(mean_logit0, mean_logit1, color=histology)) +
      geom_point() + xlab("logit0 mean")+ ylab("logit1 mean") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                                                                  ...
                                                                                  axis.line = element_line(colour = "black"), 
                                                                                  legend.position='bottom', legend.box="vertical") + guides(colour = guide_legend(nrow = 4))
    ggsave(paste("Results/NYU_test_revision/", j, "his_NYU_", i, "/out/tile_logits.png", sep=''), plot=last_plot(), width = 5, height = 5, units="in", dpi=300)
