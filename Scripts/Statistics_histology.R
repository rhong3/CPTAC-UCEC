...
previous=read.csv("~/Documents/CPTAC-UCEC/Results/Statistics_histology.csv")
existed=paste(previous$Tiles, paste(previous$Architecture, 'his', sep=''), sep='/')
sum=read_excel('~/documents/CPTAC-UCEC/Results/Summary.xlsx', sheet = 3)
inlist = paste(sum$Tiles, paste(sum$Architecture, 'his', sep=''), sep='/')
  ...
  tryCatch(
    {
      ...
      # statistical metrics
      CMP = confusionMatrix(data=results, reference=answers, positive = 'Serous')
      # ROC
      roc =  roc(answers, Test_slide$Serous_score, levels=c('Endometrioid', 'Serous'))
      rocdf = t(data.frame(ci.auc(roc)))
      colnames(rocdf) = c('ROC.95.CI_lower', 'ROC', 'ROC.95.CI_upper')
      # PRC
      SprcR = PRAUC(Test_slide$Serous_score, factor(Test_slide$True_label))
      Sprls = list()
      for (j in 1:100){
        sampleddf = Test_slide[sample(nrow(Test_slide), round(nrow(Test_slide)*0.8)),]
        Sprc = PRAUC(sampleddf$Serous_score, factor(sampleddf$True_label))
        Sprls[j] = Sprc
      }
      ...
      # statistical metrics
      CMT = confusionMatrix(data=Tresults, reference=Tanswers, positive = 'Serous')
      # ROC
      Troc =  roc(Tanswers, Test_tile$Serous_score, levels=c('Endometrioid', 'Serous'))
      Trocdf = t(data.frame(ci.auc(Troc)))
      colnames(Trocdf) = c('ROC.95.CI_lower', 'ROC', 'ROC.95.CI_upper')
      # PRC
      prcR = PRAUC(Test_tile$Serous_score, factor(Test_tile$True_label))
      prls = list()
      for (j in 1:10){
        sampleddf = Test_tile[sample(nrow(Test_tile), round(nrow(Test_tile)*0.8)),]
        prc = PRAUC(sampleddf$Serous_score, factor(sampleddf$True_label))
        prls[j] = prc
      }
...
write.csv(New_OUTPUT, file = "~/documents/CPTAC-UCEC/Results/Statistics_histology.csv", row.names=FALSE)
