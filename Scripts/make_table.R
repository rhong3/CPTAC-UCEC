...
CNVH.df$Feature = gsub("SL", "CNV-H from endometrioid", CNVH.df$Feature)
CNVH.df$Feature = gsub("CNVH", "CNV-H (Serous-like) binary", CNVH.df$Feature)
CNVH.df$Feature = gsub("CNVL", "CNV-L", CNVH.df$Feature)
...
df$Feature = gsub("CNVL", "CNV-L", df$Feature)
...
# write.csv(df, "~/documents/CPTAC-UCEC/Results/NL6/Results_table.csv", row.names=FALSE)

arch = c('I1', 'I2', 'I3', 'IR1', 'IR2', 'P1', 'P2', 'P3', 'P4', 'PC1', 'PC2', 'PC3', 'PC4')
outdf_patient = data.frame(matrix("", ncol = 13, nrow = 24))
rownames(outdf_patient) = c("Histology", "CNV-H (Serous-like) binary", "CNV-H from endometrioid", "CNV-L", "POLE", "MSI-High",  "ARID1A", "ATM",   
                            "BRCA2", "CTCF",  "CTNNB1", "FAT1",  "FBXW7", "FGFR2", "JAK1",  "KRAS",  
                            "MTOR", "PIK3CA", "PIK3R1", "PPP2R1A", "PTEN",  "RPL22", "TP53",  "ZFHX3")
colnames(outdf_patient) = arch

outdf_tile = data.frame(matrix("", ncol = 13, nrow = 24))
rownames(outdf_tile) = c("Histology", "CNV-H (Serous-like) binary", "CNV-H from endometrioid", "CNV-L", "POLE", "MSI-High",  "ARID1A", "ATM",   
                            "BRCA2", "CTCF",  "CTNNB1", "FAT1",  "FBXW7", "FGFR2", "JAK1",  "KRAS",  
                            "MTOR", "PIK3CA", "PIK3R1", "PPP2R1A", "PTEN",  "RPL22", "TP53",  "ZFHX3")
...
write.csv(outdf_patient, "~/documents/CPTAC-UCEC/Results/NL6/Results_Matrix_patient.csv")
write.csv(outdf_tile, "~/documents/CPTAC-UCEC/Results/NL6/Results_Matrix_tile.csv")

library(Hmisc)
library(Rfast)
library(dplyr)
library(pheatmap)
outdf_patient = read.csv("~/documents/CPTAC-UCEC/Results/NL6/Results_Matrix_patient.csv", row.names = 1)
outdf_tile = read.csv("~/documents/CPTAC-UCEC/Results/NL6/Results_Matrix_tile.csv", row.names = 1)
