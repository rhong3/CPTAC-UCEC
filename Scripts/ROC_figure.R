# ROC for figure
pdf(file=paste("~/documents/CPTAC-UCEC/Manuscript/Figures/ROC/ROC_tile.pdf", sep=''),
    width=5,height=5)
library(pROC)
# plot.new()
todolist = list(c('X1', 'his'), c('X2', 'SL'), c('X3', 'CNVH'), c('X2', 'CNVL'), c('X1', 'TP53'))
color = c("#55DDE0", "#33658A", "#F6AE2D", "#F26419", "#2f4858")
for (f in todolist){
  if (f[2] == 'his'){
    pos = "Serous_score"
...
legend("bottomright", legend=c("Histology", "CNV-H (Endometrioid)", "CNV-H", "CNV-L", 'TP53'),
       col=c("#55DDE0", "#33658A", "#F6AE2D", "#F26419", "#2f4858"), lwd=2)
dev.off()


# Figure 6 ROC
library(pROC)
todolist = list(c('X1histology_NYU_NL5', 'F3histology_NYU_NL6','his', 'X1', 'F3'), c('X3CNVH_NYU_NL5', 'X4CNVH_NYU_NL6', 'CNVH', 'X3', 'X4'), 
                c('X1TP53_NYU_NL5', 'I1TP53_NYU_NL6','TP53', 'X1', 'I1'), c('I5MSI_NYU_NL5', 'X4MSI_NYU_NL6', 'MSIst', 'I5', 'X4'), 
                c('X2CNVL_NYU_NL5', 'X2CNVL_NYU_NL6', 'CNVL', 'X2', 'X2'))
...
for (f in todolist){
  if (f[3] == 'his'){
    ...
    # legend("bottomright", legend=c("Mixed (0.934)", "Mixed on NYU set (0.795)", "Independent (0.87)", 'Independent on NYU set (0.833)'),
    #        col=c("#55DDE0", "#33658A", "#F6AE2D", "#F26419"), lwd=2)
    legend("bottomright", legend=c("Mixed test set (AUROC=0.889)", "NYU test set (AUROC=0.850)"),
           col=c("#F6AE2D", "#F26419"), lwd=2)
    dev.off() 
  } else{
    pdf(file=paste("~/documents/CPTAC-UCEC/Manuscript/Figures/ROC/", f[3],"_ROC_slide.pdf", sep=''),
