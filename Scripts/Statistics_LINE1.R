...
previous=read.csv("~/Documents/CPTAC-UCEC/Results/Statistics_LINE1.csv")
existed=paste(previous$Tiles, paste(paste(previous$Architecture, previous$Feature, sep=''), previous$Feature, sep='~'), sep='/')
sum=read_excel('~/documents/CPTAC-UCEC/Results/Summary.xlsx', sheet = 6)
inlist = paste(sum$Tiles, paste(paste(sum$Architecture, sum$Feature, sep=''), sum$Feature, sep='~'), sep='/')
  ...
  tryCatch(
    {
      ...
      xxx = strsplit(i, '~')[[1]][1]
      print(xxx)
      folder = xxx  #split replicated trials
...
write.csv(New_OUTPUT, file = "~/documents/CPTAC-UCEC/Results/Statistics_LINE1.csv", row.names=FALSE)
